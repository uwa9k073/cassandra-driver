...
public:
    ...
    // template<class T>
    //
    // [[nodiscard]] size_t Size() const noexcept { return
    // buffer_.size(); }
    // [[nodiscard]] std::span<std::byte> Data() noexcept { return
    // buffer_; }
    ...
    // } void WriteSmallInt(SmallInt value) {
    // WriteIntBE<SmallInt>(value); } void WriteInt(Int value) {
    // WriteIntBE<Int>(value); } void WriteBigInt(BigInt value) {
    // WriteIntBE<BigInt>(value); }
    ...
    // void WriteStringMultiMap(const std::unordered_map<std::string,
