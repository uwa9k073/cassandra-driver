...
void RequestMessage::SerializeFrame(std::vector<uint8_t>& buffer) const {
    ...
    auto container =
        std::vector<uint8_t>(buffer.begin() + header_pos, buffer.begin() + header_pos + FrameHeader::kHeaderSize);
...
void StartupMessage::Serialize(std::vector<uint8_t>& buffer) const { SerializeFrame(buffer); }
...
void QueryMessage::Serialize(std::vector<uint8_t>& buffer) const { SerializeFrame(buffer); }
...
ResponseMessage::ResponseMessage(FrameHeader header) : header_(std::move(header)) {}
...
void ResultMessage::Deserialize(const uint8_t* data, std::size_t size) {
    ...
    kind_ = static_cast<ResultKind>(
        (static_cast<int32_t>(data[0]) << 24) | (static_cast<int32_t>(data[1]) << 16) |
...
void ErrorMessage::Deserialize(const uint8_t* data, std::size_t size) {
    ...
    code_ = (static_cast<int32_t>(data[0]) << 24) | (static_cast<int32_t>(data[1]) << 16) |
            (static_cast<int32_t>(data[2]) << 8) | data[3];
